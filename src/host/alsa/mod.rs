...
impl Stream {
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        ...
        let thread = thread::Builder::new()
            ...
            .spawn(move || {
                let _ = input_stream_worker(&*stream, &mut data_callback, &mut error_callback);
            })
            .unwrap();
        Stream {
            thread: Some(thread),
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        ...
        let thread = thread::Builder::new()
            ...
            .spawn(move || {
                let _ = output_stream_worker(&*stream, &mut data_callback, &mut error_callback);
            })
            .unwrap();
        Stream {
            thread: Some(thread),
...
fn set_hw_params_from_format(
    ...
    match config.buffer_size {
        BufferSize::Fixed(v) => {
            ...
            hw_params.set_buffer_size_near((v * 4) as alsa::pcm::Frames)?;
        }
        BufferSize::Default => {
